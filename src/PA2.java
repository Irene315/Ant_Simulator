 ...
 * 
 * Description: 
 *  1. Modified function MouseDragged according to pseudocode given in lab to render the ant object.
 *  
 *  2. Modified function KeyTyped to choose parts of the ant.
 *  
 *  3. Changed the initial position of the object.
 *  
 */
...
public class PA2 extends JFrame implements GLEventListener, KeyListener, MouseListener, MouseMotionListener {
	 ...
	 * 
	 * 1 : toggle the left first leg active in rotation
	 * 
	 * 2 : toggle the right first leg active in rotation
	 * 
	 * 3 : toggle the left second leg active in rotation
	 * 
	 * 4 : toggle the right second leg active in rotation
	 * 
	 * 5 : toggle the left third leg active in rotation
	 * 
	 * 6 : toggle the right third leg for rotation
	 * 
	 * 7 : toggle the left antenna for rotation
	 * 
	 * 8 : toggle the right antenna for rotation
	 * 
	 * 9 : toggle the body for rotation
	 * 
	 * 0 : toggle the head for rotation
	 * 
	 * a : toggle all body joints of legs for rotation
	 * 
	 * b : toggle all middle joints of legs for rotation
	 * 
	 * d : toggle all distal joints of legs for rotation
	 * 
	...
	public void mouseDragged(final MouseEvent mouse) {
		if (this.rotate_world) {
			...
			// Construct rotate_axis and rotate_angle
			final float mag = (float)Math.sqrt(dx*dx + dy*dy);
			float[] rotate_axis = new float[] {};
			if (mag == 0) {
				rotate_axis = new float[] {1, 0, 0};
			}
			else {
				rotate_axis = new float[] {dy/mag, dx/mag, 0};
			}
			final float rotate_angle = 0.005f*mag;
			final float sin_val = (float)Math.sin(rotate_angle);
			final float cos_val = (float)Math.cos(rotate_angle);
			final Quaternion new_quaternion = new Quaternion(cos_val, sin_val*rotate_axis[0], sin_val*rotate_axis[1], sin_val*rotate_axis[2]);
			
			// Apply change to current quaternion
			this.viewing_quaternion = this.viewing_quaternion.multiply(new_quaternion);
